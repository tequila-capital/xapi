...
class MarketDataExample(ExampleBase):
    def __init__(self):
        super().__init__()
        ...
        try:
            for tick in response:
                if tick.Trdprc1.DecimalValue == 0.0:
                    continue
                print(
                    f'Received market data for {tick.DispName}, Last: {tick.Trdprc1.DecimalValue}'
                )
        ...
        if connect_response.Response == 'success':
            ...
            response = md_stub.SubscribeLevel1Ticks(
                md.Level1MarketDataRequest(
                    Symbols=['GE'], Request=True, Advise=True, UserToken=connect_response.UserToken
                )
            )
